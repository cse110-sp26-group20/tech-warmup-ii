...
function runSimulation() {
  console.log(
    `Starting RTP simulation for ${TOTAL_SPINS.toLocaleString()} spins...`,
  );
  console.log(
    `Wager per line: ${WAGER_PER_LINE} | Active lines: ${ACTIVE_LINES} | Total wager per spin: ${WAGER_PER_SPIN}`,
  );
  ...
  for (let i = 1; i <= TOTAL_SPINS; i++) {
    ...
    if (i % LOG_INTERVAL === 0) {
      ...
      console.log(
        `Progress: ${i.toLocaleString()} spins | Current RTP: ${currentRTP.toFixed(2)}%`,
      );
    }
  }
...
if (WAGER_PER_LINE <= 0) {
  console.error(
    'Error: WAGER_PER_LINE must be greater than 0 to calculate RTP accurately.',
  );
  process.exit(1);
}
